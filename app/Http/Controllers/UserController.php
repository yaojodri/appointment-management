...
class UserController extends Controller
{
    ...
    public function destroy(User $user)
    {
        $user->delete();
        return redirect()->route('users.index')
        ->with('alertMessage'," {$user->name} deleted successfully")->with('type', 'success');

    }

    private function getValidationRules($userId = null) {
        $rules = [
            'name' => 'required|regex:/^[\pL\s]+$/u|min:5|max:255',
        ...
        if ($userId != null){
            ...
            $rules['password'] = 'required|alpha_num|max:255|min:8|confirmed';
